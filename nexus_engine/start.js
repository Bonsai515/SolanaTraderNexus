...
function connectToSolana() {
  try {
    ...
    log(`Failed to connect to RPC: ${error.message}`);
    throw error;
  }
}

// Check wallet balance
async function checkWalletBalance(connection, walletAddress) {
  try {
...
async function initializeEngine() {
  try {
    ...
    // Load configurations
    const walletConfig = loadConfig('wallet_config.json');
    const tradingParams = loadConfig('trading_parameters.json');
    const engineConfig = loadConfig('engine_config.json');
    const rpcConfig = loadConfig('rpc_config.json');
    ...
    // Start mock trading loop (in a real implementation, this would connect to exchanges)
    startTradingLoop(tradingParams, engineConfig);
    
    return true;
  } catch (error) {
    log(`❌ Error initializing engine: ${error.message}`);
    return false;
  }
}

// Start trading loop
function startTradingLoop(tradingParams, engineConfig) {
