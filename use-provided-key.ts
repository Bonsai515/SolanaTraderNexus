/**
 * Configure System to Use Provided Private Key
 * 
 * This script sets up the trading system with the private key
 * provided by the user for real blockchain trading.
 */
...
// Private key provided by user (bs58 encoded)
const PRIVATE_KEY_BS58 = '24JWmKFhKtDec3ttegZGQeyDdMDDzY9yCW9YJwSbeCZKuNmSDzis5iCepo9bNgpwf3fLEMM7UusASxd24zWNEjuY';
...
function createKeypairFromBs58PrivateKey(bs58PrivateKey: string): Keypair {
  try {
    const decodedKey = bs58.decode(bs58PrivateKey);
...
function storePrivateKeyForNexusEngine(walletKeypair: Keypair): boolean {
  try {
      ...
      wallets: [
        {
          ...
          type: 'trading',
          label: 'User Provided Trading Wallet'
        }
      ],
...
function updateSystemMemory(walletKeypair: Keypair): boolean {
  try {
...
// Update wallets.json to add the new wallet
function updateWalletsJson(walletKeypair: Keypair): boolean {
  try {
    ...
    const newWallet = {
      ...
      isActive: true,
      profitShare: 95,
      routedTo: "31kB9NF5fTVoDAf1Tu7EcMNFx8gUHHk4cuL56bcFxk2e"
    };
    ...
    const existingIndex = wallets.findIndex((w: any) => w.publicKey === walletKeypair.publicKey.toString());
...
async function main() {
  console.log('=============================================');
  console.log('🚀 CONFIGURING SYSTEM WITH USER-PROVIDED PRIVATE KEY');
  console.log('=============================================\n');
  
  try {
    // Create keypair from provided private key
    console.log('Creating keypair from provided private key...');
    const walletKeypair = createKeypairFromBs58PrivateKey(PRIVATE_KEY_BS58);
    const walletAddress = walletKeypair.publicKey.toString();
    console.log(`✅ Keypair created successfully with address: ${walletAddress}`);
    
    // Check wallet balance
    const balance = await checkWalletBalance(walletKeypair.publicKey);
    
    // Update system with configuration for main wallet
    console.log('\n🔄 Updating system configuration for user-provided wallet...');
    
    // Store private key for transaction engine
    storePrivateKeyForNexusEngine(walletKeypair);
    
    // Update Nexus configuration
    updateNexusConfig(walletKeypair);
    
    // Update system memory
    updateSystemMemory(walletKeypair);
    ...
    console.error('Error configuring system with user-provided wallet:', error instanceof Error ? error.message : String(error));
    return false;
  }
}
