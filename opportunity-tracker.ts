...
const TEXT_STATUS_PATH = path.join('.', 'TRADING_STATUS.txt');
...
// Interface for opportunity updates
...
function initializeTrackingLog(): boolean {
  ...
  try {
    ...
    log(`Error initializing tracking log: ${(error as Error).message}`);
    return false;
  }
}

// Initialize status log
function initializeStatusLog(): boolean {
  ...
  try {
    ...
    log(`Error initializing status log: ${(error as Error).message}`);
    return false;
  }
}

// Update opportunity tracking
function updateOpportunityTracking(): OpportunityUpdate | null {
  ...
  try {
    ...
    log(`Error updating opportunity tracking: ${(error as Error).message}`);
    return null;
  }
}

// Update system status
function updateSystemStatus(opportunityUpdate: OpportunityUpdate | null): StatusData | null {
  ...
  try {
    ...
    const trackingData: TrackingData = JSON.parse(fs.readFileSync(TRACKING_LOG_PATH, 'utf8'));
    ...
    log(`Error updating system status: ${(error as Error).message}`);
    return null;
  }
}

// Create HTML dashboard
function createHtmlDashboard(): boolean {
  ...
  try {
    ...
    // Load status data
    const statusData: StatusData = JSON.parse(fs.readFileSync(STATUS_LOG_PATH, 'utf8'));
...
<body>
    ...
    log(`Error creating HTML dashboard: ${(error as Error).message}`);
    return false;
  }
}

// Create minimal text status file for quick reference
function createTextStatus(): boolean {
  try {
    // Load tracking data
    const trackingData: TrackingData = JSON.parse(fs.readFileSync(TRACKING_LOG_PATH, 'utf8'));
...
`;
    ...
    fs.writeFileSync(TEXT_STATUS_PATH, content);
    return true;
  } catch (error) {
    log(`Error creating text status: ${(error as Error).message}`);
    return false;
  }
}
...
async function main() {
  ...
  setInterval(() => {
    try {
      ...
      log(`Error in auto-update interval: ${(error as Error).message}`);
    }
  }, UPDATE_INTERVAL);
  
  log('Opportunity tracker started successfully');
  log(`Open ${HTML_DASHBOARD_PATH} to view the opportunity dashboard`);
  log(`Check ${TEXT_STATUS_PATH} for a quick status overview`);
...
main()
  .catch(error => {
    log(`Error in opportunity tracker: ${(error as Error).message}`);
  });