...
function loadHXWallet(): Keypair {
  ...
  try {
    ...
    const privateKeyHex = HX_WALLET_PRIVATE_KEY;
    ...
    if (keypair.publicKey.toString() !== HX_WALLET_ADDRESS) {
      throw new Error(`Generated keypair public key ${keypair.publicKey.toString()} does not match expected ${HX_WALLET_ADDRESS}`);
    }
    
    logger.info(`Successfully loaded HX system wallet: ${keypair.publicKey.toString()}`);
    return keypair;
  } catch (error) {
    logger.error(`Error loading HX wallet: ${error instanceof Error ? error.message : String(error)}`);
    throw error;
  }
}
