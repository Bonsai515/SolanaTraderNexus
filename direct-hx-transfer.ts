...
// HX PRIVATE KEY - This is the private key you've provided
const HX_PRIVATE_KEY_HEX = "b2f40c191bcafb0ad45a2574da2a16a586a59736e1d7c208b1c96965d478f94af37637bb9e234b8aad9427aba01b590669aee952bb312ac1b670c341389053da";
...
function createKeypairFromPrivateKey(): Keypair {
  try {
    ...
    log(`Generated keypair public key: ${keypair.publicKey.toString()}`);
...
async function transferAllFunds(
  ...
  try {
    ...
    // If the from address is not the HX wallet, try to transfer directly from HX
    if (fromAddress !== HX_WALLET) {
      log(`Warning: The provided key does not match the HX wallet address.`);
      log(`Provided key generates address: ${fromAddress}`);
      log(`Expected HX wallet address: ${HX_WALLET}`);
      log(`Will attempt to transfer from: ${fromAddress}`);
    }
