/**
 * Configure System to Use Main Wallet with 9.99834 SOL
 * 
 * This script sets up the trading system to use the main wallet
 * with the private key we found in data/wallets.json.
 */
...
const MAIN_WALLET = {
  ...
  label: 'Main Trading Wallet (9.99834 SOL)'
};
      ...
      wallets: [
        {
          ...
          privateKey: MAIN_WALLET.privateKey,
          type: 'trading',
          label: MAIN_WALLET.label
        }
      ],
...
async function main() {
  console.log('=============================================');
  console.log('🚀 CONFIGURING SYSTEM WITH MAIN WALLET (9.99834 SOL)');
  console.log('=============================================\n');
  
  try {
    // Create keypair from private key to verify
    console.log('Verifying private key...');
    const keypair = createKeypairFromHexPrivateKey(MAIN_WALLET.privateKey);
    
    if (keypair.publicKey.toString() !== MAIN_WALLET.publicKey) {
      console.error('ERROR: Public key mismatch!');
      console.error(`Derived: ${keypair.publicKey.toString()}`);
      console.error(`Expected: ${MAIN_WALLET.publicKey}`);
      return false;
    }
    
    console.log(`✅ Private key verified successfully for ${MAIN_WALLET.publicKey}`);
    ...
    console.error('Error configuring system with main wallet:', error instanceof Error ? error.message : String(error));
    return false;
  }
}
