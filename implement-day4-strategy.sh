...
echo -e "Must repay: $(echo "$FLASH_LOAN_AMOUNT + $FLASH_LOAN_FEE" | bc -l) SOL by end of transaction"
echo ""
...
echo -e "  Final amount after trades: 2.04 SOL"
echo ""

# Flash loan repayment
echo -e "${BLUE}STEP 3: FLASH LOAN REPAYMENT${NC}"
echo -e "Repaying flash loan to $FLASH_LOAN_SOURCE..."
TOTAL_REPAYMENT=$(echo "$FLASH_LOAN_AMOUNT + $FLASH_LOAN_FEE" | bc -l)
...
# Profit calculation
PROFIT=$(echo "2.04 - $TOTAL_REPAYMENT" | bc -l)
ROI=$(echo "$PROFIT / $FLASH_LOAN_AMOUNT * 100" | bc -l)
